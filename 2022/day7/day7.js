...
const part2 = (dataSource) => {
  ...
  const processDirectories = (dirs) => {
    availableSpace -= dirs["/"];
    for (let key in dirs) {
      if (availableSpace + dirs[key] >= 30000000) {
        if (dirs[key] < smallestOption || smallestOption == 0) {
          smallestOption = dirs[key];
        }
      }
    }
  };
  ...
  processList(arrayOfFiles);
  processDirectories(directoryTotals);
  console.log("grandTotal", smallestOption);
};
